...
export default function CreateEventPage() {
  ...
  const [aiTags, setAiTags] = useState([]); // tags suggested by AI generator
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Merge AI-suggested tags (if any) with the chosen category
      const mergedTags = Array.from(
        new Set([data.category, ...(aiTags || [])])
      );
  ...
  const handleAIGenerate = (generatedData) => {
    ...
    // Stash AI-suggested tags so they get merged in on submit
    if (Array.isArray(generatedData.tags)) {
      setAiTags(generatedData.tags);
    }

    // If the user already picked a start date/time, infer an end time
    // from the AI's suggested duration so they don't have to do the math.
    const startDate = watch("startDate");
    const startTime = watch("startTime");
    const durationHours = generatedData.suggestedDurationHours;
